...
proc ::tk::MessageBox {args} {
    ...
    frame $w.bot -bg $bg  ;# NEEDS WORK
    grid anchor $w.bot center
    pack $w.bot -side bottom -fill both
    frame $w.top -bg $bg  ;# NEEDS WORK
    ...
    label $w.msg -anchor nw -justify left -text $data(-message) \
        -foreground $fg   -bg $bg                      ;# NEEDS WORK dark
    if {$data(-detail) ne ""} {
		label $w.dtl -anchor nw -justify left -text $data(-detail) -bg $bg ;# NEEDS WORK
    }
    if {$data(-icon) ne ""} {
    if {([winfo depth $w] < 4) || $tk_strictMotif} {
        # label has no -bitmap option
        label $w.bitmap -bitmap $data(-icon) -bg $bg -fg $fg;# NEEDS WORK dark
    } else {
        switch $data(-icon) {
                error {
                    label $w.bitmap -image ::tk::icons::error -bg $bg
                }
                info {
                    label $w.bitmap -image ::tk::icons::information -bg $bg
                }
                question {
                    label $w.bitmap -image ::tk::icons::question -bg $bg
                }
                default {
                    label $w.bitmap -image ::tk::icons::warning -bg $bg
                }
        }
    }
    }
    ...
    foreach but $buttons {
		...
		eval [list tk::AmpWidget button $w.$name ] $opts \
			[list -command [list set tk::Priv(button) $name]]
		$w.$name configure -bg $bg -fg $fg -highlightbackground $bg \
			-highlightcolor $fg
