...
#        [regexp {\.m[0-9]+$} $target]   ||
    if {[regexp {\.pop[0-9]+$} $target] ||
		[regexp {\.pmu[0-9]+$} $target]   ||
		 [regexp {\.mnu[0-9]+$} $target]} {
        set class Popupmenu
    }
...
proc vTcl:get_subopts_special {opts w} {
    ...
    foreach i $opts {
        ...
        lassign $i opt x x def val
        if {$opt eq "-class" || $val eq $def} { continue }
...
proc vTcl:get_subopts_special_save {opts w} {
	## Got to this version of the above proc because
        ## ::widgets::${w}::subOptions::save is set in
        ## ::menu_edit::new_item.
...
#         }
        ...
        switch $class {
          ...
          TNotebook {
            ...
            TFrame -
            TSizegrip {
				append result "if {\$vTcl(current_theme) eq \"page-legacy\"} {\n"
				append result "$vTcl(tab)ttk::style configure $class "
				append result "-background \$vTcl(actual_gui_bg)\n"
				append result "}\n"	
            }
            ...
            TButton {
                ...
                append result "-font \"\$vTcl(actual_gui_font_dft_desc)\"\n"
				append result \
"$vTcl(tab)ttk::style map $class -background \
		\[list disabled $vTcl(actual_gui_bg) !disabled $vTcl(actual_gui_bg)\]\n"
...
"    ttk::style map TCombobox -background \
        \[list !disabled $vTcl(actual_gui_bg) disabled $vTcl(actual_gui_bg)\] \
        -arrowcolor \
       \[list disabled $vTcl(actual_gui_fg) !disabled $vTcl(actual_gui_bg)\]\n" 
			}
			Custom {
            }
        }
    ...
    } elseif {$vTcl(copy) && $vTcl(copy_class) eq "Popupmenu" && $len == 3} {
		regexp {^\.top\d+} $target top_part
		# set new_name [vTcl:new_widget_name "popup" $top_part]
		set new_name [vTcl:new_widget_name "popup" \$top]
		append result $new_name
		set vTcl(last_widget_name) $new_name
    ...
    foreach {co val} $class_opt {
        ...
        if {$vTcl(copy)} {
			append result $vTcl(tab) \
			  "# Following code generated in vTcl:dump_widget_opt in dump.tcl\n"
    ...
    # Put out vTcl:DefineAlias statement
...
proc vTcl:dump_widget_alias {target basename} {
	...
	# if {$vTcl(copy) && $c eq "Popupmenu"} { }  palette
	if {$vTcl(copy) && $c eq "Popupmenu" \
			&& [info exists vTcl(last_widget_name)]} {
		# Function called from vTcl:dump_widget_op.
...
proc vTcl:dump:gather_widget_info {} {
    ...
    foreach child $children {
		...
		if {$c eq "TNotebook"} {
			set pages [$child tabs]
			foreach page $pages {
				set image [$child tab $page -image]
				if {$image ne ""} {lappend used_image $image}
			}
		}			
