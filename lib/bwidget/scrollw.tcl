...
proc ScrolledWindow::create { path args } {
	...
	### I want to control the colors in the scrollbars; so the
	### following changes were made. I can do this because the
	### bwidgets are only used in PAGE windows not in the constructed
	### GUI. I do not support bwidgets in PAGE.
    ...
    # }
		...
		-background $::vTcl(pr,bgcolor)\
            -relief	 sunken
    # BWidget::get3dcolor located in utils.tcl
	set clist [BWidget::get3dcolor  $path.hscroll  $::vTcl(pr,bgcolor)]
	$path.hscroll configure -troughcolor [lindex $clist 0]
	$path.vscroll configure -troughcolor [lindex $clist 0]
    ################ End of scrollbar hack ########################
